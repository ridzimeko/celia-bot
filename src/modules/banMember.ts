...
composer
    ...
    .use(botCanRestrictUser, async (ctx: MyContext) => {
        ...
        try {
            ...
            if (reply_msg.from?.id === ctx.me.id) return await ctx.reply('Kenapa saya harus blokir diri saya sendiri?');
            ...
            if (from_user.status === 'administrator') return await ctx.reply('Maaf, saya tidak bisa memblokir admin lain');
            if (from_user.status === 'kicked') return await ctx.reply('Pengguna sudah dikeluarkan dari grup');
